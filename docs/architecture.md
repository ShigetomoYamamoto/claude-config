...
│  │   rules/autorun-flow.md — 関門付きフロー定義          │    │
...
┌────────────────────────────────────────────────────────────┐
│                     ~/.claude/                              │
│  agents/ commands/ rules/ skills/ hooks/ workflows/ settings.json │
└────────────────────────────────────────────────────────────┘
...
| ファイル | 責務 |
|---|---|
| `rules/autorun-flow.md` | 関門付き自走のフロー定義（モード表・遷移表・関門4点・success_test）。`/autorun` が解釈する |
...
2. 静的ディレクトリ（agents/commands/rules/skills/hooks/workflows）の **symlink 化**（repo を編集すれば即 live に反映。実体ディレクトリは `~/.claude/.backup/` に退避してからリンク）
...
| `GITHUB_PERSONAL_ACCESS_TOKEN` 未設定 | install.py の preflight では検知しない。MCP 起動時に Docker 側が失敗 |
